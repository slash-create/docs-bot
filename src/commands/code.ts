...
import { ephemeralResponse as _, numLength, trimContent } from '&common/helpers';
import { libraryOption, lineNumbersOption, queryOption, shareOption, versionOption } from '&discord/command-options';
...
export default class CodeCommand extends SlashCommand {
  ...
  async autocomplete(ctx: AutocompleteContext): Promise<AutocompleteChoice[] | void> {
    const { subCommands: [command], focused, focusedOption, options } = getCommandInfo(ctx);

    if (!options.library && ctx.focused !== 'library') return [responses.select];

    switch (focused) {
      case 'library': {
        return Provider.filter(options.library)
      ...
      case 'query': {
        ...
        return typeNavigator[command === 'entity' ? 'filterEntity' : 'filterFile'](focused).map((value) => {
  ...
  async run(ctx: CommandContext): Promise<MessageOptions | void | string> {
    const { subCommands: [subCommand], options } = getCommandInfo(ctx);

...
export default class UtilsCommand extends SlashCommand {
  ...
  async autocomplete(ctx: AutocompleteContext): Promise<AutocompleteChoice[]> {
    ...
    if (subGroup === 'convert') {
      switch (ctx.focused) {
        ...
        case 'to': {
          results = filter(options[ctx.focused], grouped[subCommand], {
            extract: (input) => plural(options.amount, input.singular, input.plural, false)
          }).map((result) => ({
      ...
      if (ctx.focused === 'to') {
        const originIndex = results.findIndex((opt) => opt.value === options.from);
        if (originIndex >= 0) results.splice(originIndex, 1);

        const bestUnit = measure(options.amount).from(options.from).toBest();
        const bestIndex = results.findIndex((opt) => opt.value === bestUnit.unit);
  ...
  async run(ctx: CommandContext): Promise<any> {
    ...
    if (subGroup === 'convert') {
      ...
      const commandMention = command([this.commandName, ...ctx.subcommands], this.ids.get('global'));
      const commandString = `/${this.commandName} ${ctx.subcommands.join(' ')} ${hashMapToString(options, ': ', ' ')}`;

      return _(
        [`${amount} ${fromUnit.abbr} -> ${result} ${toUnit.abbr}`, `${commandMention} - \`/${commandString}\``].join(
          '\n'
        )
      );
    }

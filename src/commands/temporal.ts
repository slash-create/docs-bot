...
import { isEmpty, plural } from "&common/helpers";
import {
...
export default class TemporalCommand extends BaseCommand {
  ...
  #runTemporalNow(ctx: CommandContext, options: TemporalNowOptions): string {
    ...
    const offsetTimeHours = options.timezone
      ? offsetTimeTo(options.timezone, invokedTime)
      : 0;
    const offsetTime = new Date(invokedTime.valueOf() + offsetTimeHours);
    const timeZoneNote =
      options.timezone && offsetTimeHours !== 0
        ? `The provided ${referenceString.split(" ").at(0)} was offset for \`${options.timezone
          }\`
         with a difference of **${plural(
            offsetTimeHours / TIME.HOUR,
            "hour",
          )}**. - *Relative to UTC*`.replace(/\s+/g, " ")
        : "";
  ...
  #runTemporalParse(
    ...
    const offsetHours = adjustedTimezone
      ? offsetTimeTo(adjustedTimezone, dateInstant)
      : 0;
    const offsetInstant = new Date(dateInstant.valueOf() + offsetHours);
    ...
    const results = chrono
      ...
      .map((entry) => {
        let ret = this.#showAndTell(
          shortTime(entry.start.date().valueOf() + offsetHours),
        );
        if (entry.end)
          ret += ` until ${shortTime(
            entry.end.date().valueOf() + offsetHours,
          )}`;
    ...
    return [
      `The ${select} **${plural(results.length, "result")}** from your query. ${instant ? `(Relative to ${shortTime(offsetInstant)})` : ""
      }`,
  ...
  #runTemporalExact(
    ...
    const exactOffsetHours = adjustedTimezone
      ? offsetTimeTo(adjustedTimezone, exactUTC)
      : 0;
    const exactOffset = new Date(exactUTC.valueOf() + exactOffsetHours);
    const isFuture = exactOffset.valueOf() > ctx.invokedAt;

    const timeZoneNote =
      options.timezone && exactOffsetHours !== 0
        ? `The provided arguments were offset for \`${options.timezone}\`
         with a difference of **${plural(
          exactOffsetHours / TIME.HOUR,
          "hour",
        )}**. - *Relative to UTC*`.replace(/\s+/g, " ")
        : "";

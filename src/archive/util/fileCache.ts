...
export class FileCache {
  ...
  static buildRawURL = (ref: string, path: string) =>
    `${rawContentLink}/${ref}/${path.replace(/#.*/, "")}`;
   ...
   */
  has = (path: string, ref: string = BRANCH): boolean =>
    this.cache.has(`${ref}:${path}`);

  get = (path: string, ref: string = BRANCH): CacheInfo =>
    this.cache.get(`${ref}:${path}`);
